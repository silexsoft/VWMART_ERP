...
const OutletDetail = () => {
  ...
  interface Bank {
    ...
    IfscCode: string;
    SwiftCode: string;
    CreditBalance: number | null; // Allow null values
  ...
  useEffect(() => {
    const fetchWarehouse = async () => {
      try {
        ...
        // setGeoCords([data.latitude, data.longitude]);
        // if (cords.length === 0) {
        //   setGeoCords([data.latitude, data.longitude]);
        // }
      } catch (error) {
  ...
  const updateAddress = async (address: any) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_HOST}/api-backend/Address/Update`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    ...
    if (warehouse && warehouse.address_id !== undefined) {
      // Ensure `address_id` exists
      fetchWarehouseAddress(Number(warehouse.address_id));
    } else {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    try {
      ...
      const respons = await UpdateWarehouse(warehouse);
