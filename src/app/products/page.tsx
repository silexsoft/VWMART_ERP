...
const ProductsPage = () => {
  ...
  const fetchProducts = async (
    ...
    try {
      ...
      &pageSize=${pageSize}&categoryIds=${categoryid}&manufacturerIds=${manufacturerid}&storeId=0&vendorId=0&
      warehouseId=${warehouseId}&visibleIndividuallyOnly=false&excludeFeaturedProducts=false&productTagId=0
  ...
  useEffect(() => {
    ...
    if (filterCategory) {
      ...
      setSelectedcategory([Number(filterCategory)]);
    } else {
      filterCategory = categoryId;
    }

    var filterManufacturer = localStorage.getItem("FilterManufacturer");
    if (filterManufacturer) {
      setShowFilters(true);
      setManufacturerId(filterManufacturer);
      setselectedmanufacturer([Number(filterManufacturer)]);
    } else {
      filterManufacturer = manufacturerId;
    }

    var filtersearchQuery = localStorage.getItem("FilterSearchProduct");
    ...
    fetchProducts(
      ...
      Number(filterManufacturer),
      filtersearchQuery,
    );
    // Fetch orders when the page index changes

    fetchCategories();
    fetchManufactures();

    localStorage.setItem("FilterCategory", categoryId);
    localStorage.setItem("FilterManufacturer", manufacturerId);
    localStorage.setItem("FilterSearchProduct", searchQuery);
  ...
  const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    localStorage.setItem("FilterSearchProduct", "");
  };
  ...
  const handleManufacturerChange = (event: any) => {
    ...
    localStorage.setItem("FilterManufacturer", "");
  };
  ...
  return (
    <DefaultLayout>
      ...
      <Breadcrumb pageName="Products" links={[{ label: "", route: "/" }]} />
