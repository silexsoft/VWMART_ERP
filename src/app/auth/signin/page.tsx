...
//   title: "Signin Page VWMart Erp",
//   description: "Signin Page VWMart Erp",
// };
...
const SignIn: React.FC = () => {
  ...
  const handleInputChange = (
    ...
    setloginCredentials((prevCredentials: any) => ({
      ...prevCredentials, // Spread the previous state
  ...
  const handleUpdate = async () => {
    try {
      const response = await signin(loginCredentials);
    } catch (error) {
  ...
  const signin = async (loginCredentials: any) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_HOST}/api-backend/Authenticate/GetToken`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        setTimeout(() => {
          setErrorMessage(false); // Hide error message after 5 seconds
        }, 5000);
      ...
      if (data.Customer.WarehouseAdmin.toString() != "0") {
        if (data.Customer) {
          localStorage.setItem(
            "NEXT_PUBLIC_BACKEND_WAREHOUSEID",
            data.Customer.WarehouseAdmin.toString(),
          );
        }

        if (data.TokenResponse.token) {
          login(data.TokenResponse.token, data.Customer.WarehouseAdmin);
          localStorage.setItem(
