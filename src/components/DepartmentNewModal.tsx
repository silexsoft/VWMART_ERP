...
interface DepartmentNewModalProps {
  show: boolean;
  handleClose: () => void;
}
...
const DepartmentNewModal: React.FC<DepartmentNewModalProps> = ({ show, handleClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formData.Image) {
        ...
        requestData.append("ImageFile", formData.Image); // Attach the image file
        requestData.append("Name", formData.Name); // Attach the department name
  
        // Make the POST request
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_HOST}/api-backend/Product/InsertDepartment`,
          {
            method: "POST",
            headers: {
